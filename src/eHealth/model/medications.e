...
feature {EHEALTH} -- commands
	...
	require
		...
		valid_string: model.is_valid_string (medicine.name)
...
feature -- public queries
	...
	medication_name_used(medication_name: STRING): BOOLEAN
		require
			valid_string: model.is_valid_string (medication_name)
		do
		...
		do
			...
			across ordered_medications as medicine loop
				...
				if attached medication_list.item (medicine.item) as medicine_tuple then
					...
					if (model.is_pill(medicine_tuple.kind)) then
						Result := Result + ",pl"
					elseif (model.is_liquid(medicine_tuple.kind)) then
						Result := Result + ",lq"
					end
	...
	format_medication(medication_id: INTEGER) : STRING
		...
		do
			...
			if attached medication_list.item (medication_id) as medication then
				...
				if (model.is_pill(medication.kind)) then
					Result := Result + ",pl"
				elseif (model.is_liquid(medication.kind)) then
					Result := Result + ",lq"
				end
