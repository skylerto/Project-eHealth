...
feature {NONE} -- Initialization
	...
	make(an_etf_cmd_name: STRING; etf_cmd_args: TUPLE; an_etf_cmd_container: ETF_ABSTRACT_UI_INTERFACE)
		do
			...
			if
				attached {INTEGER} etf_cmd_args[1] as id and then attached {STRING} etf_cmd_args[2] as name and then attached {INTEGER} etf_cmd_args[3] as kind
			then
...
feature -- command precondition 
	add_physician_precond(id: INTEGER ; name: STRING ; kind: INTEGER): BOOLEAN
		do  
			Result := 
				         comment ("ID_MD = INTEGER")
